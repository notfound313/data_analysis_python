...
def calculate_demographic_data(print_data=True):
    ...
    lower_education_rich = round(lower_education.value_counts()['>50K']/ lower_education.count()*100,1)


    # What is the minimum number of hours a person works per week (hours-per-week feature)?
    min_work_hours = df.loc[df['hours-per-week'] == df['hours-per-week'].min(), 'hours-per-week'].min()

    # What percentage of the people who work the minimum number of hours per week have a salary of >50K?
    num_min_workers = df.loc[df['hours-per-week'] == min_work_hours, 'salary'].count()

    rich_percentage = round(df.loc[df['hours-per-week'] == min_work_hours, 'salary'].value_counts()['>50K']/num_min_workers*100)

    # What country has the highest percentage of people that earn >50K?
    percentage = df.loc[df['salary'] == '>50K', 'native-country'].value_counts()/df['native-country'].value_counts()*100
