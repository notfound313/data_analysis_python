...
def draw_cat_plot():
    ...
    graf = sns.catplot(y='total', x='variable',col='cardio',hue='value',kind='bar',data=df_cat)
...
def draw_heat_map():
  ...
  (df['height'] <= df['height'].quantile(0.975)) & (df['weight'] >= df['weight'].quantile(0.025)) & (df['weight'] <= df['weight'].quantile(0.975))]
    ...
    sns.heatmap(corr, mask=mask, vmax=.3, center=0.08, square=True, annot=True,linewidths=.5,fmt='.1f',cbar_kws={"shrink": .5})
